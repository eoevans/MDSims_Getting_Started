...
def calc_inter(u, all_hydrogens, hydrogen_indices, indices, n_ts, logfile):
    ...
    rejected_hydrogens = []
    # iterate over selected spins
    for i in indices:
        # choose a spin
        selH = all_hydrogens.select_atoms('index '+str(hydrogen_indices[i]))
        # this line ensures the selH object is a 'atom' and not an 'atom group' 
        selH = selH[0]
        # how many molecular pairs will each H atom have. Any spin that does not come within the cutoff distance will not be considered. 
        ...
        # iterate over the sections of pairs
