...
# for use with a single trajectory.

def graph_acfs(Acf_intra, Acf_inter, tau_intra, tau_inter, spacing):
...
def write_voi(file, acf):
    voi = values_of_interest(acf)
    with open(file, 'a') as f:
        f.write('Tau (ps) = ' + str(voi[0]) + '\n')
        f.write('dw (kHz) = '+str(voi[1])+'\n')
        f.write('T12 = ' + str(voi[2]) + '\n')
