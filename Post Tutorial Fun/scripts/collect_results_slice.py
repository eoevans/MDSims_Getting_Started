...
def check_singer(mols, table_type):
    ref_mols = ['C5H12','C6H14','C8H18','C10H22','C12H26','C14H30','C16H34','C18H38','C20H42']
    ...
    for i in mols:
        ...
        if count == 0:
            return no_singer
    return {'molecule': ref_mols, 'T12 (s) us': np.zeros(len(ref_mols)), 'T12 (s) Singer': np.array([14.34,10.31,5.40,3.13,1.79,1.25,0.84]),
            'T12,R (s) us': np.zeros(len(ref_mols)),'T12,R (s) Singer': np.array([24.26,16.38,7.64,4.14,2.35,1.58,1.04]), 
            'T12,T (s) us': np.zeros(len(ref_mols)),'T12,T (s) Singer': np.array([35.06,27.81,18.47,12.72,7.55,5.95,4.44]), 
            'tauR (ps) us': np.zeros(len(ref_mols)),'tauR (ps) Singer': np.array([0.77,1.15,2.50,4.66,8.27,12.40,18.96]),  
            'tauT (ps) us': np.zeros(len(ref_mols)),'tauT (ps) Singer': np.array([2.32,2.89,4.35,6.30,10.62,13.66,18.24]),  
            'omegaR (kHz) us': np.zeros(len(ref_mols)),'omegaR (kHz) Singer': np.array([20.17,20.05,19.93,19.83,19.80,19.71,19.66]), 
            'omegaT (kHz) us': np.zeros(len(ref_mols)),'omegaT (kHz) Singer': np.array([9.66,9.72,9.73,9.74,9.73,9.66,9.66])}

def make_T12_table(folders, rows, table_type, table_folder, table_name, slice_start, slice_size):
    make_single_dir('MDSim/data/numbers_and_graphs/'+table_folder+'_'+slice_start+'_'+slice_size+'/')
    if table_type == 'mols':
        table_type = 'molecule'
    ...
    for i in range(len(folders)):
        ...
        for j in range(len(folders[i])):
            ...
            # read in acfs
            Acf_intra_indiv = np.load(folders[i][j]+slice_start+'/intra_acf_MPI.npy')*CONSTANT
            Acf_inter_indiv = np.load(folders[i][j]+slice_start+'/inter_acf_MPI.npy')*CONSTANT
        ...
        csv_dir = make_single_dir('MDSim/data/numbers_and_graphs/'+table_folder+'_'+slice_start+'_'+slice_size+'/'+table_name+'_csvs/')
    ...
    with open('MDSim/data/numbers_and_graphs/'+table_folder+'_'+slice_start+'_'+slice_size+'/'+table_name+'_T12_avg.txt', 'w') as f:
        f.write(tabulate(table_sum, headers = 'keys', numalign='center', stralign='center'))
        dt_string = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
        f.write('\ncreated '+dt_string)
    with open('MDSim/data/numbers_and_graphs/'+table_folder+'_'+slice_start+'_'+slice_size+'/'+table_name+'_T12_indiv.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    if results['table_rows'] == 'mols':
        ...
        for i in range(len(sim_folders)):
            try:
                make_T12_table(sim_folders[i], mols, results['table_rows'], automate['sim_folder'], params_folders[1][i], sys.argv[2], sys.argv[3])
            except:
        ...
        for i in range(len(sim_folders)):
            try:
                make_T12_table(sim_folders[i], params_folders[1], results['table_rows'], automate['sim_folder'], mols[i], sys.argv[2], sys.argv[3])
            except:
                print('error in T12 table')
                traceback.print_exc()
            try:
