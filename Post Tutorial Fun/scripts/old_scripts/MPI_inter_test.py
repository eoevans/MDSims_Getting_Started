...
def three_theta(point, points, sideLen):
    v = np.absolute(points-point)
    v = v - np.trunc(v/sideLen+0.5)*sideLen
    az = v[:,2]
...
def calc_inter(u, all_hydrogens, sub_hydrogens, n_ts, lag_fraction, logfile):
    ...
    for i in np.arange(len(sub_hydrogens)):
        ...
        for ts in u.trajectory[0:n_ts-1]:
            pairs_vals[ts.frame,:] = three_theta(selH.position, pairs_positions.positions, side_len)
        acf_local += calc_single_acf(pairs_vals,lag_fraction)
...
mol_fraction = 2 #1/fraction of H atoms to survey
side_len = 40 #Angstroms
...
inter_acf_local = np.empty(int(lag_fraction*n_ts)+1)
...
if rank == 0:
    ...
    rand_sel = np.sort(rand_sel[0:n_choose//mol_fraction])
...
inter_acf_local = calc_inter(u, all_hydrogens, sub_hydrogens, n_ts, lag_fraction, fileout) # restore this part for other numpy save file arguments , sys.argv[6]+"_"+str(rank)+".npy", sys.argv[7]+"_"+str(rank)+".npy")
...
if Comm.Get_rank() == 0:
    ...
    with open(sys.argv[5], 'a') as f:
        f.write('script called: MPI_inter_test.py\n')
        ...
        f.write('fraction of hydrogens used = '+str(1/mol_fraction)+'\n\n')
        f.write('number of hydrogen atoms = '+str(len(all_hydrogens)))
