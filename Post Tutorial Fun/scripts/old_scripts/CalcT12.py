...
def graph_acfs(Acf_intra, Acf_inter, t_intra, t_inter):#    Acf_intra = Acf_intra[:len(t_intra)]
...
if __name__ == '__main__':
    Acf_intra = np.load(sys.argv[1])
    Acf_inter = np.load(sys.argv[2])
    ...
    t_intra = calc_time_scale(framespacing, tau_intra)
    ...
    graph = True
    try:
        graph_acfs(Acf_intra,Acf_inter,t_intra,t_inter)
    except ValueError:
