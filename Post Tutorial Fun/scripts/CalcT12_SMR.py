...
def graph_acfs(Acf_intra, Acf_inter, tau_intra, tau_inter, spacing):#    Acf_intra = Acf_intra[:len(t_intra)]
    ...
    plt.savefig(files['acf_log'][:-4]+'acf_graph')
...
def calc_J(acf):
    return np.fft.fft(even(acf)).real
...
if __name__ == '__main__':
    ...
    JR = calc_J(Acf_intra)*0.1
    JT = calc_J(Acf_inter)*0.1
    freqs = np.fft.fftfreq(JT.size, framespacing)
    tauRJ = 0.5*JR[get_index(freqs, 0)]/Acf_intra[0]
    tauTJ = 0.5*JT[get_index(freqs, 0)]/Acf_inter[0]

    print(str(tauRJ)+' '+str(tau_intra))
    print(str(tauTJ)+' '+str(tau_inter))

    larmor_freq = (19.44 * 10**6) * 10**-12 # in ps^-1
    T1R = JR[get_index(freqs, larmor_freq)] + 4*JR[get_index(freqs, larmor_freq*2)]
    T2R = 1.5*JR[get_index(freqs, 0)] + 2.5*JR[get_index(freqs, larmor_freq)] + JR[get_index(freqs, 2*larmor_freq)]
    T1T = JT[get_index(freqs, larmor_freq)] + 4*JT[get_index(freqs, larmor_freq*2)]
    T2T = 1.5*JT[get_index(freqs, 0)] + 2.5*JT[get_index(freqs, larmor_freq)] + JT[get_index(freqs, 2*larmor_freq)]
    T1 = (T1T**-1 + T1R**-1)*10**-12
    T2 = (T2T**-1 + T2R**-1)*10**-12
    #print(T1)
    #print(T2)
    T12 = ((10*(tau_intra*Acf_intra[0] + tau_inter*Acf_inter[0]))**-1)/(10**12)
    #print(T12)
    print(JR[get_index(freqs, 0)])
    print(tau_intra*2*Acf_intra[0])
